...
int    nl_update_count = 0;    // verlet list update counter
...
inline void apply_pbc(double* r) { // periodic boundary conditions
  for (int dim = 0; dim < 3; ++dim) {
    if (r[dim] >= lbox[dim])
      r[dim] -= lbox[dim];
    else if (r[dim] < 0.0)
...
// Euler algorithm
  for (int i = 0; i < n_particles; ++i) {
		for (int dim = 0; dim < 3; ++dim) {
		  ...
		  pos[i][dim] += vel[i][dim] * md_delt;		// positon at t+dt
			dis[i][dim] += vel[i][dim] * md_delt;
		  vel[i][dim] += fff[i][dim] * md_delt;	// velocity at t+dt
      			fff[i][dim] = 0.0;
		}
		apply_pbc(pos[i]);
  }
  update_neigh_list();
	...
	/*for (int i = 0; i < n_particles; ++i)
		for (int dim = 0; dim < 3; ++dim)
			vel[i][dim] += fff[i][dim] * md_delt_half;  // velocity at t+dt(velocity-Verlet algorithm)*/
}
